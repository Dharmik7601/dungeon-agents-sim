...
- `SemanticLogger.flush(run_id)` — serialises `_events` to `data/run_{run_id}_{ts}.json`, creates the `data/` directory if missing, returns the file path.

**`src/tracing/langfuse_wrapper.py`**

- `wrap_with_langfuse(llm_client, run_id, turn_number, agent_id)` — returns a decorated `get_decision` callable. If `LANGFUSE_PUBLIC_KEY` and `LANGFUSE_SECRET_KEY` env vars are absent, returns the original method unchanged (transparent no-op). If present, wraps with `@observe(name="agent_turn_execution")` and tags the trace with run metadata.
...
- **Integration** — known `stale_shadow_state` mismatch appears in `execution_result.deltas` of the logged event; Langfuse wrapper falls back gracefully when env vars absent
